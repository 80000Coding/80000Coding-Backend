...
import io.oopy.coding.common.utils.jwt.AuthConstants;
...
import io.oopy.coding.common.utils.redis.refresh.RefreshToken;
...
public class JwtAuthorizationFilter extends OncePerRequestFilter {
    ...
    private String resolveAccessToken(HttpServletRequest request, HttpServletResponse response) throws ServletException {
        String authHeader = request.getHeader(AuthConstants.AUTH_HEADER.getValue());
        try {
    ...
    private String handleExpiredToken(HttpServletRequest request, HttpServletResponse response) throws ServletException {
        ResponseCookie cookie = cookieUtil.deleteCookie(request, response, AuthConstants.REFRESH_TOKEN.getValue())
    ...
    private String reissueAccessToken(HttpServletRequest request, HttpServletResponse response) throws ServletException {
        try {
            Cookie refreshTokenCookie = cookieUtil.getCookie(request, AuthConstants.REFRESH_TOKEN.getValue())
                    .orElseThrow(() -> new AuthErrorException(AuthErrorCode.REFRESH_TOKEN_NOT_FOUND, "Refresh token not found"));
            ...
            ResponseCookie cookie = cookieUtil.createCookie(AuthConstants.REFRESH_TOKEN.getValue(), reissuedRefreshToken.getToken(), refreshTokenCookie.getMaxAge());
            response.addHeader(HttpHeaders.SET_COOKIE, cookie.toString());
            ...
            response.addHeader(AuthConstants.ACCESS_TOKEN.getValue(), reissuedAccessToken);
