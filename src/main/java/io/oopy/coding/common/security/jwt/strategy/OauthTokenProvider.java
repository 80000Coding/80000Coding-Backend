...
import static io.oopy.coding.common.security.jwt.JwtField.GITHUB_ID;
import static io.oopy.coding.common.security.jwt.JwtField.ROLE;

@Slf4j
@Component
@OauthRegisterTokenQualifier
public class OauthTokenProvider implements JwtProvider {
    ...
    public JwtSubInfo getSubInfoFromToken(String token) {
        ...
        return JwtOauthInfo.builder()
                .githubId(claims.get(GITHUB_ID.getValue(), Integer.class))
                .role(roleType)
                .build();
    }
    ...
    private Map<String, Object> createClaims(JwtSubInfo dto) {
        return Map.of(GITHUB_ID.getValue(), dto.id(),
